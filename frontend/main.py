...
RESOURCE = os.environ.get(
    ...
    "projects/157718358816/locations/us-east1/reasoningEngines/3005372098014085120",
)
...
def _extract_parts(parts: list) -> list[dict]:
    ...
    for p in parts:
        ...
        if text_val and isinstance(root, TextPart):
            if "Cannot add session to memory" not in text_val:
        ...
        if data_val is not None:
            if isinstance(data_val, dict):
                # Wrapped A2UI part: {"kind": "data", "metadata": {"mimeType": "application/json+a2ui"}, "data": {...}}
                meta = data_val.get("metadata") or {}
                if meta.get("mimeType") == _A2UI_MIME and "data" in data_val:
                    out.append({"kind": "a2ui", "data": data_val["data"]})
                    continue
                # Direct A2UI message
                if "surfaceUpdate" in data_val or "beginRendering" in data_val:
                    out.append({"kind": "a2ui", "data": data_val})
                    continue

            root_meta = getattr(root, "metadata", None) or {}
            mime = root_meta.get("mimeType") if isinstance(root_meta, dict) else None
            if mime == _A2UI_MIME:
                out.append({"kind": "a2ui", "data": data_val})
                continue
    ...
    async with httpx.AsyncClient(headers=_auth_headers(), timeout=120) as client:
        ...
        async for event in a2a_client.send_message(msg):
            ...
            if isinstance(update, TaskArtifactUpdateEvent) and getattr(update, "artifact", None):
                parts.extend(_extract_parts(getattr(update.artifact, "parts", [])))
        ...
        # Check history if no parts collected yet
